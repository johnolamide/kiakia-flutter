...
import 'schema/item_sold_record.dart';
...
export 'schema/item_sold_record.dart';
...
/// Functions to query ItemSoldRecords (as a Stream and as a Future).
Future<int> queryItemSoldRecordCount({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      ItemSoldRecord.collection(parent),
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<ItemSoldRecord>> queryItemSoldRecord({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
    ...
    queryCollection(
      ItemSoldRecord.collection(parent),
      ItemSoldRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<ItemSoldRecord>> queryItemSoldRecordOnce({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
...
Future maybeCreateUser(User user) async {
  final userRecord = BuyersRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await BuyersRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createBuyersRecordData(
    email: user.email ??
  ...
  currentUserDocument = BuyersRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createBuyersRecordData(email: email));
}
